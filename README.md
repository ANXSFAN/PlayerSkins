...
> ⚠️ **换刀须知**：改刀之前**必须先用 `!knife <刀名>` 选一把真实的刀**（如 `!knife karambit`），不要直接对**默认刀**用 `!skin` / `!seed` 等指令——默认刀无法换皮，强行修改会触发客户端致命断言崩溃（`weapon_knife script file not found`）。本插件已拦截该情况，但请遵循此用法。
